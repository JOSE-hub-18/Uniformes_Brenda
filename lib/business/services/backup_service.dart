...
class BackupService {
  ...
  BackupService({required BackupRepository backupRepository})
  ...
  Future<BackupResult> crearBackup() async {
    try {
      final ruta = await _backupRepository.crearBackup();
      ...
      return BackupResult(exitoso: false, mensaje: 'Error al crear backup: $e');
    }
  }
  ...
  Future<BackupResult> crearYCompartirBackup() async {
    final resultado = await crearBackup();

    if (!resultado.exitoso || resultado.backup == null) {
      return resultado;
    }

    try {
      await Share.shareXFiles(
        [XFile(resultado.backup!.ruta)],

        subject: 'Backup Uniformes Brenda',
      ...
      return BackupResult(
        ...
        mensaje: 'Backup listo para compartir.',
      ...
      return BackupResult(
        ...
        mensaje: 'Backup creado pero no se pudo compartir: $e',
  ...
  Future<List<BackupInfo>> listarBackups() async {
    try {
      final directorio = await _directorioBackups();

      if (!await directorio.exists()) {
        return [];
      }

      final archivos = directorio.listSync().whereType<File>().where((f) {
        return basename(f.path).startsWith('backup_uniformes_') &&
            f.path.endsWith('.db');
      }).toList();
  ...
  Future<BackupResult> compartirBackup(BackupInfo backup) async {
    try {
      final archivo = File(backup.ruta);

      if (!await archivo.exists()) {
        return const BackupResult(
          exitoso: false,

          mensaje: 'El backup no existe.',
        );
      }
      ...
      await Share.shareXFiles(
        ...
        text: 'Respaldo: ${backup.nombre}',
      );

      return BackupResult(
        exitoso: true,

        mensaje: 'Backup compartido.',

        backup: backup,
      );
    } catch (e) {
      return BackupResult(exitoso: false, mensaje: 'Error al compartir: $e');
    }
  }
  ...
  Future<BackupResult> eliminarBackup(BackupInfo backup) async {
    try {
      final archivo = File(backup.ruta);
      ...
      return BackupResult(exitoso: false, mensaje: 'Error al eliminar: $e');
    }
  }
  ...
  Future<void> _limpiarBackupsAntiguos() async {
    ...
    final aEliminar = backups.sublist(_maxBackups);

    for (final backup in aEliminar) {
      await eliminarBackup(backup);
    }
  }

  /// Retorna el directorio donde se almacenan los backups.
  /// Si el directorio no existe, lo crea de forma recursiva.
  Future<Directory> _directorioBackups() async {
    final base = await getApplicationDocumentsDirectory();

    final dir = Directory(join(base.path, 'backups'));
