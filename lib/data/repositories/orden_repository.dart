...
class OrdenRepository {
  ...
  Future<int> insertar(Orden orden) async {
    final db = await _db;

    return await db.insert('ordenes', orden.toMap()..remove('id'));
  }

  /// Retorna la orden con el identificador indicado.
  /// Retorna null si no existe ninguna orden con ese id.
  Future<Orden?> obtenerPorId(int id) async {
    final db = await _db;
  ...
  Future<List<Orden>> obtenerTodas() async {
    final db = await _db;

    final maps = await db.query('ordenes', orderBy: 'fecha DESC');

    return maps.map((m) => Orden.fromMap(m)).toList();
  }
}
