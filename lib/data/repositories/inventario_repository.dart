...
class InventarioRepository {
  ...
  Future<int> contarStock(int idInventario) async {
    ...
    final result = await db.rawQuery(
      'SELECT COUNT(*) as total FROM unidades WHERE id_inventario = ? AND activo = 1',
      [idInventario],
    );
  ...
  Future<List<Map<String, dynamic>>> obtenerStockAgotado() async {
    ...
    return await db.rawQuery('''
      SELECT e.nombre AS escuela, p.nombre AS prenda,
             t.talla, i.precio,
             (SELECT COUNT(*) FROM unidades u WHERE u.id_inventario = i.id AND u.activo = 1) AS stock
      FROM inventario i
      ...
      WHERE (SELECT COUNT(*) FROM unidades u WHERE u.id_inventario = i.id AND u.activo = 1) = 0
    ''');
  }
  ...
  Future<List<Map<String, dynamic>>> obtenerStockCritico() async {
    ...
    return await db.rawQuery('''
      ...
      JOIN tallas t ON i.id_talla = t.id_talla
      WHERE (SELECT COUNT(*) FROM unidades u WHERE u.id_inventario = i.id AND u.activo = 1) > 0
        AND (SELECT COUNT(*) FROM unidades u WHERE u.id_inventario = i.id AND u.activo = 1) <= 3
    ''');
  }

  // filtro dinamico por escuela y opcionalmente prenda
